...
# --- УМНАЯ ЗАГРУЗКА ТОКЕНА ---
# Ищем .env в папке со скриптом или на уровень выше
...
if not TOKEN:
    print(f"❌ ОШИБКА: Токен не найден в файле: {env_path}")
    print("Убедитесь, что в .env прописано: BOT_TOKEN=ваш_токен")
    exit(1)

# Твои актуальные ID чатов
CHAT_CURATOR = -1003550048093
CHAT_STRESS_TEST = -1003584211374
CHAT_ADMIN_APPLY = -1003686254634

# --- БАЗА ДАННЫХ ---
# Создаем базу в папке со скриптом, чтобы она не терялась
...
async def handle_curator_msg(message: types.Message):
    ...
    await message.answer("✅ Отправлено куратору.")
...
async def ban_user(message: types.Message):
    ...
    except: await message.reply("Пример: /ban 12345678")
...
async def handle_reply(message: types.Message):
    ...
    if message.chat.id == CHAT_CURATOR:
        ...
        if not cur_id:
            ...
            await message.answer(f"🤝 Вы закрепились за учеником.")
...
async def main():
    print(f"🚀 Бот запущен! Используется база: {db_path}")
